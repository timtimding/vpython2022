...
class CO_molecule:
    ...
    def com(self): # return position of center of mass
        return (self.O.pos * self.O.m + self.C.pos * self.C.m) / (self.C.m + self.O.m)
    def com_v(self): # return velocity of center of mass
        return (self.O.v * self.O.m + self.C.v * self.C.m) / (self.C.m + self.O.m)
    def v_P(self): # return potential energy of the bond for the vibration motion
        return self.bond.k / 2 * (mag(self.bond.axis) - d) ** 2 
    def v_K(self): # return kinetic energy of the vibration motion
        return (dot((self.O.v - self.com_v()), self.bond.axis.norm()))** 2 * self.O.m / 2 \
             + (dot((self.C.v - self.com_v()), self.bond.axis.norm()))** 2 * self.C.m / 2
    def r_K(self): # return kinetic energy of the rotational motion
        return self.O.m / 2 * mag(self.O.v - self.com_v()) ** 2 \
             + self.C.m / 2 * mag(self.C.v - self.com_v()) ** 2 \
             - self.v_K()
    def com_K(self): #return kinetic energy of the translational motion of the center of mass
        return (self.O.m + self.C.m) / 2 * mag(self.com_v()) ** 2
def collision(a1, a2):
